...
import {
    ...
    CssBaseline, makeStyles, Grid, Typography
...
let Menu = (props) => {
    ...
    let navs = props.nav.map((nav) => {
        return <Button key={nav.id}
                       className={classes.button}>{props.currentLanguage === 'EN' ? nav.title_EN : nav.title_RU}</Button>
    });
            ...
            <AppBar position="fixed" className={classes.appBar}>
                <Toolbar>
                    ...
                    <Grid container justify='left'>
                        <Grid key={1} className={classes.logo_wrapper} item>
