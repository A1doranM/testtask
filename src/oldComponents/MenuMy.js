...
const useStyles = makeStyles(theme => ({
    root: {
        display: 'grid',
        height: '20vh',
        gridTemplateAreas: 'logo nav languages controls',
...
let Menu = (props) => {
    ...
    let navs = props.nav.map((nav) => {
        return <Button key={nav.id} className={classes.button}>{props.currentLanguage === 'EN' ? nav.title_EN : nav.title_RU}</Button>
    });
